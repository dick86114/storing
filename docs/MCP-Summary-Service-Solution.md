...
Storing 当前定位是 AI 驱动的个人稍后阅读平台，核心能力包括链接采集、AI 摘要、智能分类、自动标签和归档。
...
当前系统更接近单用户产品，`articles` 和 `article_metadata` 的数据边界不足以支撑对外开放。如果直接把现有 `/collect` 暴露给 MCP，会造成外部请求大量进入个人业务流，污染用户自己的收件箱、归档和搜索。
