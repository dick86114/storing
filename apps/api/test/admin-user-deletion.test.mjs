...
test('删除用户必须在事务内处理关联数据，并保留全局 articles', () => {
  ...
  assert.match(route, /user_id: result\.user\.id/);
  assert.match(route, /user: result\.user/);
...
test('Android 与 Web 都必须把用户名确认传给删除接口', () => {
  const androidApi = readWorkspace('apps/android/app/src/main/java/com/idickies/storing/admin/AdminApi.kt');
  const androidRepository = readWorkspace('apps/android/app/src/main/java/com/idickies/storing/admin/AdminRepository.kt');
  ...
  assert.match(androidScreen, /onDelete = \{ confirmation -> viewModel\.deleteUser\(user\.id, confirmation\) \}/);
  assert.match(androidScreen, /onClick = \{ onDelete\(confirmation\.trim\(\)\) \}/);
});
