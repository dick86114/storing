...
test('分类 API、文章归档和 AI 受控分类使用同一套分类契约', async () => {
  const [index, articleRoutes, aiService, collectService, categoryService] = await Promise.all([
  ...
  assert.match(articleRoutes, /c\.req\.queries\('tag'\)/);
  assert.match(articleRoutes, /applyArchiveTagFilter/);
  assert.match(articleRoutes, /articlesRoutes\.get\('\/tags'/);
