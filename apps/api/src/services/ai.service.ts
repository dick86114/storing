...
export async function classifyArticle(articleId: number, title: string, summary: string, aiSummary: string | null): Promise<void> {
  const system = 'You are an article classifier. Respond with ONLY a single category name from this list: 技术, 设计, AI, 商业, 职业, 效率';
  const user = `Classify this article into exactly one category:
...
Respond with only the category name, nothing else.`;
  ...
  const category = (await callAI(system, user)).trim();
...
export async function classifyAndTag(articleId: number): Promise<void> {
  ...
  await classifyArticle(articleId, title, summary, metaWithSummary?.aiSummary || null);
