...
Web 端需要新增一个一级栏目用于呈现 Wiki 内容。该栏目应符合 Wiki 的典型信息架构：可浏览、可交叉链接、可追溯来源、可持续增长，而不是简单文章列表。
...
| 页面 | 路由建议 | 说明 |
|------|----------|------|
| Wiki 首页 | `/wiki` | 展示知识库概况、主题目录、最近更新、任务状态 |
| Wiki 页面详情 | `/wiki/[slug]` | 展示某个主题/概念的结构化文档 |
| Wiki 搜索/问答 | `/wiki/search` 或首页内弹窗 | 第一版先做搜索入口，第二阶段接问答 |
| Wiki 管理状态 | `/wiki/status` 或设置内嵌 | 展示编译任务、失败任务、手动更新 |

### 12.0.1 Wiki 首页布局

Wiki 首页应明显区别于收件箱/归档页。它不是卡片瀑布流，而是知识库门户。

桌面端推荐布局：

```text
┌──────────────────────────────────────────────────────────────┐
│ 顶部导航：Logo / 收件箱 / 收藏 / 归档 / Wiki / 搜索 / 用户     │
├──────────────────────────────────────────────────────────────┤
│ Wiki Header                                                  │
│ - 标题：我的知识库 / 玉简阁 / Wiki                           │
│ - 副标题：由 N 篇归档文章整理而成，最近更新于...              │
│ - 操作：更新知识库 / 查看任务 / 提问知识库                    │
├──────────────────────┬───────────────────────────────────────┤
│ 左侧目录/筛选          │ 主内容区                              │
│ - 全部页面             │ - 知识地图/主题概览                    │
...
│ - 状态筛选             │                                       │
├──────────────────────┴───────────────────────────────────────┤
│ 来源与状态摘要：已编译文章、待处理文章、失败任务、上次更新时间 │
└──────────────────────────────────────────────────────────────┘
```
...
- 顶部保留当前主题的移动导航风格
- Header 区展示知识库名称、文章数量和更新时间
- 下方使用横向 tabs 或 segmented control 切换：
  - 概览
  - 主题
  - 最近
  - 状态
- 页面列表采用紧凑的 Wiki 条目，而不是文章卡片大封面
- 更新知识库按钮放在 Header 操作区或更多菜单中

### 12.0.2 Wiki 页面详情布局

Wiki 页面详情应采用文档阅读布局，突出结构、引用和内部链接。
...
- 页面标题、摘要、更新时间优先展示
- 来源文章、相关页面、大纲折叠为抽屉或底部 sheet
- 正文 block 单列展示
- 每个重要要点可点击查看来源
- 底部操作栏提供：来源、相关页面、重新生成、分享

### 12.0.3 Wiki 条目与文档排版

Wiki 内容应使用“知识条目”而不是普通文章卡片。
...
每个 block 应预留来源元数据展示能力。第一版可以在段落或要点右侧/下方展示来源计数，点击后展开来源文章列表。
...
Wiki 栏目需要适配 Storing 现有主题体系。页面视觉应贴合当前主题风格，不能出现与其它页面割裂的默认样式。

通用原则：

- 复用现有主题变量、字体、间距、圆角、阴影和背景策略
- Wiki 是文档型页面，视觉密度应高于文章卡片页，但低于设置/管理页
- 不直接复用文章卡片样式作为 Wiki 条目，应设计更符合知识库的条目形态
- 来源、引用、反链、页面状态等 Wiki 元素要有一致的视觉语言
- 移动端需要保持轻量，避免大面积 blur、复杂背景动画或重绘压力

各主题适配方向：

| 主题 | Wiki 视觉方向 |
|------|---------------|
| 微信 | 清爽实用，类似公众号资料库；使用白底、浅分割线、清晰列表与来源标签 |
| 玻璃 | 半透明面板、柔和高光；控制 blur 强度，文档正文保持足够阅读对比 |
| 极光 | 轻盈渐变和柔彩背景；正文区域使用更稳定的浅色承载层，避免影响阅读 |
| 杂志 | 编辑部/目录式排版；强调标题层级、分栏目录、纸感卡片和编号结构 |
| 修仙 | 玉简/藏书阁语义；可使用山水纹理、玉牌式状态、卷轴/目录结构，但必须保持文档可读性 |

Wiki 主题元素建议：

- Wiki 首页 Header 可根据主题使用不同命名和装饰，但信息结构保持一致
- 主题页大纲可以作为“目录”“卷目”“章节”等主题化表达
- 来源引用可以根据主题呈现为“来源文章”“参考资料”“引文”“本卷来源”等
- 状态标识需要保留统一语义，不因主题变化而难以理解
...
开发和验收时，如果当前归档文章数量不足，可以临时从收件箱中选择部分文章归档，用于构造 Wiki 测试数据。

要求：

- 测试归档行为必须走正式归档流程，确保触发 Wiki job 的真实链路
- 测试前后记录被归档的文章，便于必要时恢复
- 不应直接伪造 Wiki 页面绕过文章抽取流程
- 测试数据应覆盖不同类型文章：技术、工具、生活/消费、资讯、长文、短文
- 取消归档和删除文章的测试也需要覆盖，验证 Wiki reconcile 行为
- 如果使用生产样例数据测试，避免调用不可控高成本模型；优先使用 DeepSeek v4-flash 或配置的低成本模型
...
- Wiki 页面版本会被保存
- 后续问答所需的数据结构已预留
- Wiki 栏目具有独立的信息架构和排版，不是归档页样式的简单复用
- Wiki UI 在微信、玻璃、极光、杂志、修仙主题下均符合各自主题风格
- 归档文章不足时，可以通过正式归档收件箱文章完成端到端测试
